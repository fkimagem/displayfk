...
Thermometer thermometer(225, 75, 0);
...
void loop(){
    // ------- ESCRITO MANUAL
    int raw = analogRead(potPin); //leitura bruta de 0 a 4095
    int ranged = map(raw,0,4095,0, 100); //valor mapeado de 0 a 100 para entrar no termometro
    thermometer.setValue(ranged); //Use this command to change widget value.
    // thermometer.setValue() criado automaticamente, apenas argumento "ranged" foi inserido
    // ------- FIM
    delay(2000);
}
