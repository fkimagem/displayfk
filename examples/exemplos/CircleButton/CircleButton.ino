...
CircleButton cirbtn(240, 160, 0);
const uint8_t qtdCircleBtn = 1;
CircleButton *arrayCirclebtn[qtdCircleBtn] = {&cirbtn};
bool cirbtn0_val = false; // Global variable that stores the value of the widget cirbtn
// ------- ESCRITO MANUAL
int pinLed = 16;
// ------- FIM
...
void loop(){
    // Use the variable cirbtn0_val to check value/status of widget cirbtn
    delay(2000);
}
...
void cirbtn_cb(){
    cirbtn0_val = cirbtn.getStatus();
    // ------- ESCRITO MANUAL
    digitalWrite(pinLed, cirbtn0_val );
    // ------- FIM
}
