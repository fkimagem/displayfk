...
TextBox textbox(145, 140, 0);
const uint8_t qtdTextbox = 1;
TextBox *arrayTextbox[qtdTextbox] = {&textbox};
const char* tb0_val = ""; // Global variable that stores the value of the widget textbox
// ------- ESCRITO MANUAL
int pinLed = 16;
// ------- FIM
...
void loop(){
    // Use the variable tb0_val to check value/status of widget textbox
    delay(2000);
}
...
void textbox_cb(){
    ...
    // ------- ESCRITO MANUAL
    bool on = String(tb0_val).equalsIgnoreCase("ON"); //compara se e igual a "ON" ignora maiusculas ou minusculas
    digitalWrite(pinLed, on ? HIGH : LOW); //liga ou desliga o led de acordo
    // ------- FIM
}